...
class MyApp(QMainWindow):
    def __init__(self):
        ...
        self.ui.btn_en_3.clicked.connect(self.call_api_gen_keys)
        self.ui.btn_en.clicked.connect(self.call_api_encrypt)
        self.ui.btn_de.clicked.connect(self.call_api_decrypt)
        self.ui.btn_en_2.clicked.connect(self.call_api_sign)
        self.ui.btn_de_2.clicked.connect(self.call_api_verify)
    ...
    def call_api_sign(self):
        ...
        payload = {
            "message": self.ui.txt_plain_2.toPlainText()
        }
        try:
            response = requests.post(url, json=payload)
            if response.status_code == 200:
                data = response.json()
                self.ui.txt_cipher_2.setText(data["signature"])
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Information)
    ...
    def call_api_verify(self):
        ...
        payload = {
            "message": self.ui.txt_plain_2.toPlainText(),
            "signature": self.ui.txt_cipher_2.toPlainText()
        }
        try:
            response = requests.post(url, json=payload)
            if response.status_code == 200:
